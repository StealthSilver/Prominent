...
const WatchList: React.FC = () => {
  const data = {
    ...
    datasets: [
      {
        label: "Price",
        data: watchlist.map((stock: Stock) => stock.price),
        backgroundColor: [
          "rgba(255, 99, 132, 0.5)",
          "rgba(54, 162, 235, 0.5)",
          "rgba(255, 206, 86, 0.5)",
  ...
  return (
    <div className="watchlist-container">
      ...
      <ul className="list">
        {watchlist.map((stock: Stock, index: number) => (
...
const WatchListItem: React.FC<WatchListItemProps> = ({ stock }) => {
  ...
  const handleMouseEnter = (_e: MouseEvent<HTMLLIElement>) => {
    setShowWatchlistActions(true);
  };

  const handleMouseLeave = (_e: MouseEvent<HTMLLIElement>) => {
